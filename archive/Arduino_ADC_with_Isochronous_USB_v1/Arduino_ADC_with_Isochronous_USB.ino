...
//  } else {
  ...
  for (int i = 0; i<1023;i++) {
    USBISO.writebuffer[i] =  250;//USBISO.ADC_Bfr[i];
  }
...
void loop() {
    ...
    if(  RTC->MODE0.COUNT.bit.COUNT < 40000000)
    {
      while (ADC->INTFLAG.bit.RESRDY==0);
        
      //USBISO.writebuffer[j]=*ADCregL; (j>=1023) ? 0 : j++;
      //USBISO.writebuffer[j]=*ADCregH; (j>=1023) ? 0 : j++;
      USBISO.writebuffer[j]=250;(j>=1023) ? 0 : j++;
      //USBISO.ADC_reg[0].b8[0]=*ADCregL;//127;//
      //USBISO.ADC_reg[0].b8[1]=*ADCregH;//0;//
      //SerialUSB.print(USBISO.ADC_reg[0].b16);
      USBISO.Continue=1;
      
      Iteration++;
    } else
    {
      SerialUSB.println("Dit is het:");
